...
class JdepsTask {
    ...
    private boolean run() throws IOException {
        ...
        for(Archive archive : archives) {
        	...
        	List<Archive> dependencies = new ArrayList<>();
        	
        	analyzer.visitArchiveDependences(archive, new Analyzer.Visitor() {
				
				@Override
				public void visitDependence(String origin, Archive source, String target,
						Archive archive, Profile profile) {
					// TODO Auto-generated method stub
					
				}
				
				@Override
				public void visitArchiveDependence(Archive origin, Archive target,
						Profile profile) {
					if(!"rt.jar".equals(target.getFileName())) {
        	...
        	if(!dependencies.isEmpty()) {
	        	...
	        	for(Archive dependency : dependencies) {
    ...
    private static class ResourceBundleHelper {
        ...
        static {
            ...
            try {
                bundle = ResourceBundle.getBundle("resources.jdeps", locale);
            } catch (MissingResourceException e) {
                throw new InternalError("Cannot find jdeps resource bundle for locale " + locale);
            }
            try {
                versionRB = ResourceBundle.getBundle("resources.version");
            } catch (MissingResourceException e) {
